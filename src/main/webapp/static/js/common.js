...
var path10= "http://120.77.213.58:8080/cg/"; //行数腾讯云;
...
$(document).ready(function () {
    ...
    $('ul#nav-tabs a').click(function () {
        ...
        for (let i = 0; i < $divData.length; i++) {
            ...
            } else if ($dataFunction === "total"){
                totalTableFunction($dataUrl, $dataDiv, $dataAbbreviation);
            } else{
    ...
    // 监听规格输入框
    $("input.size-input-editor").click(function(e){
        productSizeEditor($(this));
        e.stopPropagation();
    });
    $("div#product-size-editor").click(function(e){
        e.stopPropagation();
    });
    $(document).click(function(){
        $("div#product-size-editor").addClass("hide");
    })

    //订单填写用户名字获取相应的联系电话和配送地址
    $("#add-order-customer-name").blur(function(){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                targetObject = iterationObject(map.result[0], targetObject);
                $("#add-order-customer-phone").val(targetObject["customerPhone"]);
                $("#add-order-customer-address").val(targetObject["customerAddress"]);
            }
        })
})
...
function getStaffModalModifyData($inputForm) {
    var modifyData = new Map();
...
function commonTableFunction(_dataUrl, _dataDiv, _dataAbbreviation) {
    ...
    //近一周
    $("#" + _dataAbbreviation + "-aWeek").unbind('click').on('click', function () {
        var mydate = new Date();
    ...
    //近两周
    $("#" + _dataAbbreviation + "-twoWeeks").unbind('click').on('click', function () {
        var mydate = new Date();
    ...
    //近一个月
    $("#" + _dataAbbreviation + "-aMonth").unbind('click').on('click', function () {
        var mydate = new Date();
...
//通用函数
function CreateTable(divName, targetId, _url, _startTime, _endTime, _init, pageType) {
    var $id=targetId;
...
function initAjaxUseGet(_url, _dataType, _contentType, divName, pageType, _startTime, _endTime) {
    var json,
        ...
        $id = 1; //初始页面
    CreateTable(divName, $id, _url, _startTime, _endTime, "init", pageType);
}
...
function customerCreateTable(divName, targetId, _url, _name, _init, pageType) {
    var $id=targetId;
    var $tbody = $(divName + " .table-content tbody");
    $tbody.html("");
    var tableName = [];
...
function customerInitAjaxUseGet(_url, _dataType, _contentType, divName, pageType, _name) {
    var json,
        ...
        $length, //返回json数据记录总数量
        $id = 1; //初始页面
    customerCreateTable(divName, $id, _url, _name, "init", pageType);
}
...
function materialInventoryTableFunction(_dataUrl, _dataDiv, _dataAbbreviation) {
    ...
    //筛选事件
    $("#material-inventory-refresh").unbind('click').on('click', function () {
        materialInventoryAjax("" + path + _dataUrl, "json", "application/json;charset=utf-8", "" + _dataDiv);
    })
}
...
function inventoryCreateTable(divName, targetId, _url, _productModel, _productSize, _productShape, _init, pageType) {
    var $id=targetId;
    var $tbody = $(divName + " .table-content tbody");
    $tbody.html("");
    var tableName = [];
    ...
    if (_init === "init") {
        ...
        if (length > 0) {
            ...
            showStatusModal(":-", "该 产品 查询没有记录")
        }

        //点击页数
        $(divName + " ." + pageType + " .page").on("click", function () {
            $id = parseInt($(this).attr('id').match(/\d+(\.\d+)?/g)); //当前为第几页
            inventoryClickPage(divName, $id, $page, $(this), _url, _productModel, _productSize, _productShape);
        });

        // 点击上一页
        $(divName + " ." + pageType + " .last").on("click", function () {
            $id = inventoryClickLastPage(divName, $id, $page, _url, _productModel, _productSize, _productShape);
        });

        // 点击下一页
        $(divName + " ." + pageType + " .next").on("click", function () {
            $id = inventoryClickNextPage(divName, $id, $page, _url, _productModel, _productSize, _productShape);
        })
    }
    for (let i = 0; i < length; i++) {
        $tbody.append('<tr id="record-table-row-' + (i + 1) + '"></tr>');
...
function inventoryInitAjaxUsePost(_url, _dataType, _contentType, divName, pageType, _productModel, _productSize, _productShape) {
    var json,
        ...
        $id = 1, //初始页面
        _data = {};
    inventoryCreateTable(divName, $id, _url, _productModel, _productSize, _productShape, "init", pageType);
...
function orderTableFunction(_dataUrl, _dataDiv, _dataAbbreviation) {
    ...
    //筛选事件
    $("#for-select").unbind('click').on('click', function () {
...
function orderCreateTable(divName, targetId, _url, _startTime, _endTime, _orderNum, _customerName, _orderstatus, _init, pageType) {
    var $id=targetId;
    var $thead = $(divName + " .table-content thead");
    var $tbody = $(divName + " .table-content tbody");
    $tbody.html("");
    ...
    $(".table-content-detail").on('click', function () {
        ...
        var target = {};
        target = iterationObject(map.result[id], target);
        var inputs=$(".order-detail").find("input");
        $("#order-modal").modal("show")
            .on("shown.bs.modal", function () {
                for (let j = 0; j < tableName.length; j++) {
                     let reg = /.{0,}Date.{0,}/;
                    if (reg.test(tableName[j])) {
                        var date = new Date(target[tableName[j]]).toLocaleString(target[tableName[j]]);
                        target[tableName[j]] = date.split(" ")[0];
                        var changeDate = target[tableName[j]].split("-");
                        for (let z = 1; z < changeDate.length; z++) {
                            var temp = parseInt(changeDate[z]);
                            if (temp < 10) {
                                changeDate[z] = "0" + changeDate[z];
                        ...
                        target[tableName[j]] = changeDate.join("-");
                    }
                    inputs.eq(j).val(target[tableName[j]]);
                }
            })
    });
...
function orderInitAjaxUseGet(_url, _dataType, _contentType, divName, pageType, _startTime, _endTime, _orderNum, _customerName, _orderstatus) {
    var json,
        ...
        $length, //返回json数据记录总数量
        $id = 1;//初始页面
    orderCreateTable(divName, $id, _url, _startTime, _endTime, _orderNum, _customerName, _orderstatus, "init", pageType);
...
function statisticsCreateTable(divName, _url, _startTime, _endTime) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        for (let j = 1; j < tableName.length; j++) {
            ...
            if(tableName[j]==="purchaseQuantitySum"){
                totalMaterialPurchase=totalMaterialPurchase+parseFloat(targetObject[tableName[j]]);
            }
            if(tableName[j]==="totalPriceSum"){
                totalTotalPrice=totalTotalPrice+parseFloat(targetObject[tableName[j]]);
            }
            $tbody.children("tr:last-child").append('<td>' + targetObject[tableName[j]] + '</td>');
        }
    }
    $tbody.append('<tr></tr>');
    if (divName === ".material-purchase-statistics-table") {
        $tbody.children("tr:last-child").append('<td>合计</td><td>'+totalMaterialPurchase+'</td><td>' + totalTotalPrice +'</td>');
    }
}
...
//总出库量和总金额
function totalTableFunction(_dataUrl, _dataDiv, _dataAbbreviation){
    //初始页面
    var mydate = new Date();
    ...
    var todayTime = todayTimeFormat(mydate);
    var today = Date.parse(todayTime);
    totalInitAjaxUseGet("" + path + _dataUrl, "json", "application/json;charset=utf-8", "" + _dataDiv, today, today);
    ...
    $("#" + _dataAbbreviation + "-select").unbind('click').on('click', function () {
        ...
        if ($selectStartTime > $selectEndTime) {
            ...
            totalInitAjaxUseGet("" + path + _dataUrl, "json", "application/json;charset=utf-8", "" + _dataDiv, $selectStartTime, $selectEndTime);
        }
    })
    ...
    $("#" + _dataAbbreviation + "-threeDays").unbind('click').on('click', function () {
        ...
        var today = Date.parse(todayTime);
        var otherday = today - 172800000;
        totalInitAjaxUseGet("" + path + _dataUrl, "json", "application/json;charset=utf-8", "" + _dataDiv, otherday, today);
    })
    ...
    //所有统计事件
    $("#" + _dataAbbreviation + "-total").unbind('click').on('click', function () {
        var $selectStartTime = Date.parse($("#" + _dataAbbreviation + "-start-date").val());
        var $selectEndTime = Date.parse($("#" + _dataAbbreviation + "-end-date").val());
        if ($selectStartTime > $selectEndTime) {
            showStatusModal("请选择正确的时间");
        } else {
            totalInitAjaxUseGet("" + path + _dataUrl, "json", "application/json;charset=utf-8", "" + _dataDiv, 0, today);
        }
    })
}
function totalInitAjaxUseGet(_url, _dataType, _contentType, divName, _startTime, _endTime){
    $.ajax({
        url:_url,
        type:"GET",
        data:{
        ...
        contentType:_contentType,
        success:function(data){
            if(divName === ".order-total-amount-statistics-table"){
                console.log("test");
                console.log(data.result);
                $(""+divName+" .table-content th[data-total='target']").html("");
                $(""+divName+" .table-content th[data-total='target']").append(data.result);
            }else if(divName === ".warehouse-out-storage-total-weight-statistics-table"){
                $(""+divName+" .table-content td[data-total='totalInstorage']").html("");
                $(""+divName+" .table-content td[data-total='totalDelivery']").html("");
                $(""+divName+" .table-content td[data-total='totalInstorage']").append(data.result.totalInstorage);
                $(""+divName+" .table-content td[data-total='totalDelivery']").append(data.result.totalDelivery);
            } else{
                $(""+divName+" .table-content td[data-total='produceTotal']").html("");
                $(""+divName+" .table-content td[data-total='outStorageTotal']").html("");
                $(""+divName+" .table-content td[data-total='produceTotal']").append(data.result.totalProductQuantity);
                $(""+divName+" .table-content td[data-total='outStorageTotal']").append(data.result.totalOutStorageQuantity);
            }
        }
    })
}
...
function initAdd() {
    ...
    $(".add-form-item").focus(function() {
        var errormsg = $(this).parents("div.add-form").find("span.hiddenMsg").text();
        if (errormsg == "原料占比和不为1，请检查") {} else {
            $(this).parents("div.add-form").find("span.hiddenMsg").text("").removeClass("errorMsg");
        }
    });
            ...
            $.fn.clearDatas = function () {
                ...
                switch (btnID) {
                    ...
                    case "btn-cancel-add-product": {
                        $("#add-product-model").attr("placeholder", "大写字母加数字");
        ...
        (function ($) {
            $.fn.passDatesToModal = function () {
                ...
                if (inputLen === needLen) {
                    ...
                    var errormsg = $(this).siblings("span").text();
                    //有改动
                    if (errormsg != "原料占比和不为1，请检查") {
        ...
        $("#add-product-model").blur(function () {
            ...
            if (!result) {
                ...
                if (existCode == 1) {
                    ...
                    $(this).val('').attr("placeholder", "大写字母加数字");
                }
            }
        });
            ...
            $(this).blur(function() {
                ...
                if (!result) {
                    if (ratio == '') {
                        $(this).attr("placeholder", "输入不能为空");
                        //有改动
                    } else if (Number(ratio) < 0 || Number(ratio) >= 100) {
                    ...
                    if (ratioAg && ratioCu && ratioZn && ratioCd && ratioSn) {
                        ...
                        //有改动
                        if (totalRatio != 100) {
                            $(this).parents("div.add-form").find("span").text("原料占比和不为1，请检查").addClass("errorMsg");
                            $(this).val('').attr("placeholder", "保留两位小数");
                        } else {
                            $(this).parents("div.add-form").find("span").text("").removeClass("errorMsg");
                        }
                    }
                }
            });
    ...
    $(function () {
        ...
        $("#add-order-quantity-total").blur(function () {
            ...
            //     $("#add-order-total-price").val(sellTotalPrnumber-judgeice);
            // }
        });
        ...
        //对输入联系电话进行判断
        $("input[id$=order-customer-phone]").blur(function () {
            var reg1 = /^1\d{10}$/g;
...
function initModal() {
    ...
    $(".btn-add-record-row").click(function () {
        addRecordRow($(this),_modelArr,_sizeArr,_shapeArr,_productArr,_allModelArr);
    ...
    $(".emit-modal-confirm").click(function () {
        ...
        if( isValidInput( all_records[parentModalId] ) ){
            $("#modal-confirm").modal("show")
                .on("shown.bs.modal", function () {
    ...
    $(".btn-commit-in-staff-modal").click(function () {
        ...
        var sendData = convertToJson(getStaffModalModifyData($inputModify)); //转换成JSON格式的data
        var finalData = JSON.stringify(sendData);
    ...
    // 初始化型号请求，用于开炉、胚料、清仓
    var _allModelArr = getAllModelArr();//获取全部已添加的型号
    initProductSelect($("select[data-model=product-model"),_allModelArr);//初始化包括全部的型号下拉框

    //用于产品清仓
    var _productArr = getProductObj();//获取所有产品的数组
    var _modelArr = getAllProductModel(_productArr);//获取所有产品的型号的数组
    var _sizeArr = getAllProductSize(_productArr);//获取所有产品的规格的数组
    var _shapeArr = getAllProductShape(_productArr);//获取所有产品的形态的数组
    initProductSelect($("select[data-model-cwp=product-model-cwp"),_modelArr);//初始化型号
    ...
    /*新增，当产品型号改变时触发*/
    $("select[data-model-cwp=product-model-cwp]").change(function(){
        var $model = $(this).val();
    ...
    $("select[data-size=product-size]").change(function(){
        var $modelSelected = $(this).parent().prev("td").find("select[data-model-cwp=product-model-cwp]");//找到规格框之前的型号框
...
//判断all_records[parentModalId](即recordArr)是否为有效的记录
function isValidInput(recordArr){
    for (let i = 0; i < recordArr.length; i++) {
        var perRowObj = recordArr[i];
        for(let j in perRowObj){
            if(!perRowObj[j]){
                return false;
            }
        }
...
function inputCheck(tdName) {
    ...
    $("td[data-ratio] input").prop("readonly", true);
}
...
function setMaterialConsume() {
    ...
    if (date && model) {
        ...
        $("#consume-sum").val(result.materialConsume);
        $("#waste-sum").val(result.wasteConsume);
        $("#leftover-sum").val(result.leftoverConsume);
    }
}
...
//在对应表格上添加一行记录
//在对应表格上添加一行记录
function addRecordRow(_this,_modelArr,_sizeArr,_shapeArr,_productArr,_allModelArr) {
    ...
    var indexModelCwp = $(_this).parent().parent().find("td[data-model-cwp]").index(); //获取产品清仓的型号所在的列数
    var indexRatio = $(_this).parent().parent().find("td[data-ratio]").index(); //获取开炉损耗比所在的列数
    var indexWaste = $(_this).parent().parent().find("td[data-smart='wasteConsume']").index(); //获取废料所在的列数
    var indexLeft = $(_this).parent().parent().find("td[data-smart='leftoverConsume']").index(); //获取边角料所在的列数
    var indexBlank = $(_this).parent().parent().find("td[data-smart='blankProduce']").index(); // 获取胚料产出所在的列数
    ...
    var optionStrModelCwp = optionLeft + optionMiddle + optionRight;//清仓型号的第一项，为空
    ...
    //填充产品型号的option元素
    for (let i = 0; i < _allModelArr.length; i++) {
        optionStrModel += optionLeft + _allModelArr[i] + optionMiddle + _allModelArr[i] + optionRight;
    }
    //填充产品清仓型号的option元素
    for (let i = 0; i < _modelArr.length; i++) {
        optionStrModelCwp += optionLeft + _modelArr[i] + optionMiddle + _modelArr[i] + optionRight;
    }
    //填充产品规格的option元素
    ...
    for (let i = 0; i < columns; i++) {
        ...
        }else if(i == indexModelCwp){
            $tr.append('<td data-model-cwp><select data-model-cwp="product-model-cwp">' + optionStrModelCwp + '</select></td>');
        }else if(i == indexAuto){
            $tr.append('<td data-auto><input class="input-in-td" type="text" data-smart="target"/></td>');
        }else if(i == indexSize){
        ...
        }else if(i == indexRatio){
            $tr.append('<td data-ratio><input class="input-in-td" data-smart="lossRatio" type="text"/></td>');
        }else if(i == indexWaste){
            $tr.append('<td ><input class="row-append input-in-td" data-smart="wasteConsume" type="text"/></td>');
        }else if(i == indexLeft){
            $tr.append('<td><input class="row-append input-in-td" data-smart="leftoverConsume" type="text"/></td>');
        }else if(i == indexBlank){
            $tr.append('<td><input class="row-append input-in-td" data-smart="blankProduce" type="text"/></td>');
        }else{
...
function postAjaxRequest(_parentModalId, _path, recordObj) {
    ...
    delete recordObj[_parentModalId];
    console.log("请求已发送");
}
...
//单独获取全部型号，包括不存在规格和形态的型号，用于开炉、胚料和其他清仓
function getAllModelArr(){
    var allModelArr = [];
    $.ajax({
        url: path + "ProductModelInfo/productModels",
        type: "GET",
...
function checkInputModify() {
    $(".staff-data-modify").blur(function () {
        var $parent = $(this).parent();
        $parent.find(".form-tips").remove();
...
function writeDataToStaffModal(modalId, writeData) {
    $("#" + modalId).modal("show")
        .on("shown.bs.modal", function () {
            $(this).find(".staff-data-modify").each(function (index, element) {
                var key = $(element).attr("data-key");
                $(element).val(writeData[key]);
            });
        });
    ...
    $(":input[data-smart]").blur(function () {
        ...
        switch (method) {
            case "multiplication":
                ...
                $(this).parent().siblings().find(":input[data-smart='target']").val(multip.toFixed(2));
                break;
            ...
            case "ratio":
                var RATIOSUM = Number(100).toFixed(2);
                var ratioString = $(this).val();
                if (ratioString === '') {
                    $(this).val("0.00");
                }else {
                    $(this).val(Number(ratioString).toFixed(2));
                }

                if (parseFloat($(this).val()) > 100) {
                    showRatioModifyError($(this), "比例和不可以大于100%")
                    ...
                    for (let i = 0; i < ratioInput.length; i++) {
                        ratioSum = Number(Number(ratioSum) + Number(parseFloat(ratioInput.eq(i).val()))).toFixed(2);
                        console.log("ratioSum", ratioSum);
                    }
                    if (ratioSum > RATIOSUM) {
                        showRatioModifyError($(this), "比例和不等于100%");
                    } else if (ratioSum === RATIOSUM) {
                        $("#ratioErrorMsg").text("");
                        $("button#btn-submit-product-model-modify").removeAttr("disabled").focus();
                    } else {
                        showRatioModifyError($(this), "比例和不等于100%");
                    }
            ...
            case "blank":
                ...
                } else if(productBlank>material) {
                    showStatusModal("!!!", "胚料输入量不合理。应小于三量总和");
                    $('#modal-blank-modify .btn-commit-in-modal').attr("disabled", "true");
                }else{
                    var ratio = 1-productBlank/material;
                    $(this).parent().siblings().find("[data-smart='target']").val(Number(ratio*100).toFixed(2));
                }
                break;

            case "blankProduce":
                var material = parseFloat($(this).parent().siblings().find("[data-smart='target']").val());//获取原料消耗值
                var wasteConsume = parseFloat($(this).parent().siblings().find("[data-smart='wasteConsume']").val());//获取废料值
                var leftoverConsume = parseFloat($(this).parent().siblings().find("[data-smart='leftoverConsume']").val())//获取边角料值
                var productBlank = multip - wasteConsume - leftoverConsume;//计算实际胚料产出量
                //判定输入胚料量是否大于废料和边角料总和（即实际胚料产出量是否大于零）
                ...
                if(productBlank<0){
                    ...
                    var ratio = 1-productBlank/material;
                    $(this).parent().siblings().find("[data-smart='lossRatio']").val(Number(ratio*100).toFixed(2));
                }
                break;
...
// 规格编辑器
function productSizeEditor($productSizeInput) {
    var $psi = $productSizeInput;
    $psi.blur();
    var $pse = $("#product-size-editor");
    $pse.removeClass("hide");
    $pse.offset(function(){
            var newPos = {};
            newPos.top =$psi.offset().top + $psi.height() + 15;
            newPos.left =$psi.offset().left;
            return newPos;
    });
    var preVal = $psi.val();
    var thickness,
        ...
        width;
    if (preVal === '') {
        thickness = length = width = 0.1;
    } else {
        var preValArr = preVal.split("*");
        thickness = parseFloat(preValArr[0]);
        length = parseFloat(preValArr[1]);
        width = parseFloat(preValArr[2]);
        console.log(thickness, length, width);
    }
    var $t = $pse.find("#thickness-editor");
    var $l = $pse.find("#length-editor");
    var $w = $pse.find("#width-editor");
    $t.val(Number(thickness).toFixed(1));
    $l.val(Number(length).toFixed(1));
    $w.val(Number(width).toFixed(1));

    $t.blur(function(){
        $t.val(Number($t.val()).toFixed(1));
    });
    $l.blur(function(){
        $l.val(Number($l.val()).toFixed(1));
    });
    $w.blur(function(){
        $w.val(Number($w.val()).toFixed(1));
    });

    $pse.find("#btn-confirm-editor").click(function (){
        var postVal = '';
        postVal += $t.val() + '*' + $l.val() + '*' + $w.val();
        console.log(postVal);
        $psi.val(postVal);
        $pse.addClass("hide");
    });
    $pse.find("#btn-reset-editor").click(function(){
        thickness = length = width = 0.1;
        $t.val(thickness);
        $l.val(length);
        $w.val(width);
    });
    $pse.find("#btn-cancel-editor").click(function (){
        $pse.addClass("hide");
    });
